 ...
 */
export const defaultRendererMap: TokenRendererMap = {
  [AdmonitionType]: function renderAdmonition(admonition, key, ctx) {
    const title = ctx.renderNodes(admonition.title)
    return (
      <AdmonitionRenderer key={key} keyword={admonition.keyword} title={title}>
  ...
  [CodeType]: function renderCode(code, key, ctx) {
    ...
    return (
      <CodeRenderer key={key} lang={lang} value={formattedValue} meta={meta} />
    )
  },
  ...
  [ImageReferenceType]: function renderImageReference(
    ...
    const {
      ...
      sizes,
      loading,
    } = imageReference as ImageReference & React.ImgHTMLAttributes<HTMLElement>
