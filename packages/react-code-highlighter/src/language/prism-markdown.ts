...
function createInline(pattern: string): RegExp {
  const resolvedPattern = pattern.replace(/<inner>/g, function () {
    return inner
  })
  return RegExp(/((?:^|[^\\])(?:\\{2})*)/.source + '(?:' + resolvedPattern + ')')
}
...
const tagPattern = RegExp(
  (Prism.languages.markup as Grammar & { tag: any }).tag.pattern.source,
  'gi',
)
